...
export default function LinkedInExportActions({
  optimized,
  saveStatus = "idle",
  onSave,
  headline,
  about,
}: LinkedInExportActionsProps) {
  const [copiedHeadline, setCopiedHeadline] = useState(false);
  const [copiedAbout, setCopiedAbout]       = useState(false);

  const handleCopy = (field: "headline" | "about") => {
    const text = field === "headline" ? headline : about;
    if (text) {
      ...
      <div className="flex flex-col gap-2.5">
        ...
        >
          {copiedHeadline ? (
            ...
            <><svg width="15" height="15" viewBox="0 0 15 15" fill="none"><rect x="5" y="5" width="8" height="8" rx="1.5" stroke="currentColor" strokeWidth="1.25" /><path d="M10 5V3.5A1.5 1.5 0 008.5 2h-5A1.5 1.5 0 002 3.5v5A1.5 1.5 0 003.5 10H5" stroke="currentColor" strokeWidth="1.25" /></svg>Copy LinkedIn Headline</>
          )}
        </button>
        ...
        >
          {copiedAbout ? (
            ...
            <><svg width="15" height="15" viewBox="0 0 15 15" fill="none"><rect x="5" y="5" width="8" height="8" rx="1.5" stroke="currentColor" strokeWidth="1.25" /><path d="M10 5V3.5A1.5 1.5 0 008.5 2h-5A1.5 1.5 0 002 3.5v5A1.5 1.5 0 003.5 10H5" stroke="currentColor" strokeWidth="1.25" /></svg>Copy About Section</>
          )}
        </button>
        ...
        <button
          type="button"
          disabled={!optimized || saveStatus === "saving"}
          onClick={onSave}
          className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-sm transition-all duration-200 hover:border-white/20 hover:text-white disabled:opacity-40 disabled:cursor-not-allowed"
          style={{
