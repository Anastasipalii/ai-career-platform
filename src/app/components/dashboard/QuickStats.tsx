...
function buildStats(p: QuickStatsProps): StatConfig[] {
  return [
    {
      label: "Total Resumes",
      value: String(p.resumeCount),
      trend: p.resumeCount > 0 ? `${p.resumeCount} saved` : "No resumes yet",
      color: "#7c3aed",
    ...
    {
      label: "Cover Letters",
      value: String(p.coverLetterCount),
      trend: p.coverLetterCount > 0 ? `${p.coverLetterCount} created` : "No letters yet",
      color: "#06b6d4",
    ...
    },
    {
      ...
      value: String(p.interviewCount),
      trend: p.interviewCount > 0 ? `${p.interviewCount} completed` : "No sessions yet",
      color: "#f59e0b",
    ...
    {
      label: "Job Matches",
      value: String(p.jobMatchCount),
      trend: p.jobMatchCount > 0 ? `${p.jobMatchCount} found` : "Run a job search",
      color: "#8b5cf6",
    ...
    },
    {
      ...
      value: String(p.languageCount),
      trend: p.languageCount > 0 ? `${p.languageCount} languages` : "Translate a resume",
      color: "#10b981",
    ...
    {
      label: "Avg ATS Score",
      value: p.avgAtsScore !== null ? `${p.avgAtsScore}%` : "—",
      trend: p.avgAtsScore !== null ? `Across ${p.resumeCount} resume${p.resumeCount !== 1 ? "s" : ""}` : "Build a resume first",
      color: "#ec4899",
