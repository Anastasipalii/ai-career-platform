// ============================================================================
// geo/countries — complete ISO 3166-1 country list + name→code resolution
// ----------------------------------------------------------------------------
// Pure data + helpers. Used by the Search Preferences UI (a complete country
// list, not a hardcoded handful) and by the deterministic location filter to
// resolve a job's country from its free-text location. NOTE: this is
// country/region matching only — NOT true kilometre-distance geocoding.
// ============================================================================
...
// Common aliases / native names → code (for résumé + job-location matching).
...
/** Resolve a country CODE from free text, or "" when none is confidently found.
 *  Country/region only — never a guessed city. */
export function resolveCountryFromText(raw: string | null | undefined): string {
  ...
  for (const { code, name } of COUNTRIES) {
    if (new RegExp(`\\b${name.toLowerCase().replace(/[^a-z ]/g, "")}\\b`).test(t)) return code;
  }
  return "";
}

export const isCountryCode = (code: string): boolean => CODE_SET.has((code ?? "").toUpperCase());
